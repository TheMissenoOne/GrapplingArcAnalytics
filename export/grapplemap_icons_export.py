"""Generate GrappleMap position icons (Seam A).
...
2. **App vocab subset** → ``GrapplingArcApp/src/assets/grapplemap_icons/`` — one
   representative icon PNG per matched app position node, pre-built for a deferred
   app picker feature. (The code-generated require-index, ``grapplemapIconIndex.ts``,
   was removed 2026-09-09 — it had zero importers in the App; regenerate it from
   this function's return value if the picker gets wired.)
...
matches whatever label form it passes.
