...
def test_verifier_adds_raw_user_meta_data_for_profile_provision_trigger() -> None:
    # Scaffold intentionally does not carry this column (Supabase provides it live);
    # every verify_*.py script that inserts into auth.users adds it itself instead
    # (see ci/verify_rls_isolation.py, ci/verify_owner_deletion.py). This pins that
    # our verifier follows the same convention.
    verifier_source = (ROOT / "ci/verify_user_sessions_identity.py").read_text()
    assert (
