"""O professor lê o NOME do aluno, e só o nome — e agora a faixa, por decisão própria (0057).
...
``belt_rank``/``belt_degrees`` entram na projeção pela 0057 — exatamente a "decisão de
privacidade, com revisão própria" que este docstring pedia. ``is_pro``/``archetype_id``/
``is_guest`` continuam de fora; e-mail nunca esteve em ``profiles``.

Varredura de fonte porque a suíte roda em SQLite e nunca executa migration Postgres (nota de
escopo da 0019). Frágil de propósito: se alguém acrescentar OUTRA coluna à projeção, isso falha
e a decisão de privacidade volta a ser explícita.
"""
...
def test_projeta_nome_e_faixa() -> None:
    corpo = _corpo_vigente()
    assinatura = corpo.split("as $$")[0]

    assert "profile_id uuid" in assinatura
    assert "full_name text" in assinatura
    assert "belt_rank text" in assinatura
    assert "belt_degrees integer" in assinatura

    for coluna in ("is_pro", "archetype_id", "is_guest", "email"):
...
def test_0057_recoloca_os_grants_apos_o_drop() -> None:
    """Mudar as colunas de retorno de uma função existente exige DROP FUNCTION antes do CREATE —
    `create or replace` sozinho não muda o shape de `returns table`. O DROP apaga os grants que a
    0045 concedeu, então a 0057 tem que reconceder, não só recriar o corpo."""
    arquivo = (VERSOES / "0057_group_member_belt_and_canonical.py").read_text(encoding="utf-8")

    assert "drop function if exists public.group_member_names(uuid);" in arquivo
    assert "revoke all on function public.group_member_names(uuid) from anon;" in arquivo
    assert "revoke all on function public.group_member_names(uuid) from public;" in arquivo
    assert "grant execute on function public.group_member_names(uuid) to authenticated;" in arquivo
