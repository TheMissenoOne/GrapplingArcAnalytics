...
def test_export_icons_writes_subset(gmap, tmp_path):
    ...
    # Tag + exact matching covers a solid chunk of the curated technique vocab.
    ...
    # Index TS: generated header, one require per node_key, every referenced PNG exists.
    import re

    text = index_ts.read_text(encoding="utf-8")
    assert text.startswith("// GENERATED by export/grapplemap_icons_export.py")
    assert "GRAPPLEMAP_ICONS: Record<string, number>" in text
    entries = re.findall(r'"([^"]+)": require\("\.\./assets/grapplemap_icons/([^"]+)"\)', text)
    assert len(entries) == len(matched)
    for _node_key, rel_path in entries:
        assert (app_assets / Path(rel_path).name).is_file()
...
def test_export_icons_check_mode_writes_nothing_and_detects_drift(gmap, tmp_path):
    ...
    # First real run to establish a baseline file.
    export_icons(
        full_icons_dir=full_dir, app_assets_dir=app_assets,
        app_index_ts=index_ts, size=64, verbose=False,
    )
    baseline = index_ts.read_text(encoding="utf-8")

    # check=True against an up-to-date file: no drift, nothing written.
    export_icons(app_index_ts=index_ts, verbose=False, check=True)
    assert index_ts.read_text(encoding="utf-8") == baseline

    # check=True against a stale file: raises, still writes nothing.
    index_ts.write_text("stale\n", encoding="utf-8")
    with pytest.raises(SystemExit):
        export_icons(app_index_ts=index_ts, verbose=False, check=True)
    assert index_ts.read_text(encoding="utf-8") == "stale\n"
