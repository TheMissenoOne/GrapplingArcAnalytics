"""One-time merge: fold ``technique_nodes`` rows whose stored ``node_key`` doesn't match
their label's canonical key (per the technique library's ``clean_label``) into the
canonical node, remap incident ``graph_edges``/``map_edges``, delete the fragment node,
and replay affected athlete graphs so exports read clean.

Root cause: ingest derives ``node_key = _normalize_name(raw_label)`` WITHOUT running
``clean_label`` first, so a label spelled with a hyphen ("Arm-Triangle") normalizes to a
different key ("armtriangle") than the library's space-spelled canonical ("arm triangle
choke"). Cloned from ``analysis/merge_attempt_nodes.py`` (card 012) — same remap/replay
machinery, only the node-selection + mapping differ.

    uv run python -m analysis.merge_technique_dups              # dry-run (default), no writes
    uv run python -m analysis.merge_technique_dups --apply       # write
    uv run python -m analysis.merge_technique_dups --check       # 0 = none remain, no writes

ponytail: doesn't touch ``Match.sequence`` (the raw event labels that feed replay) — if a
resolved node's matches still carry the unclean label, replaying them re-derives it from
...
def find_mismatched_nodes(session: Session) -> list[TechniqueNode]:
    """Every technique_node whose canonical key (via ``clean_label``) differs from its
    stored ``node_key`` — the fragment nodes left behind by unclean ingest."""
    out = []
    for n in session.execute(select(TechniqueNode)).scalars():
        canonical_key = _normalize_name(clean_label(n.label or ""))
        if canonical_key != n.node_key:
            out.append(n)
    return out


def plan_merges(nodes: list[TechniqueNode]) -> list[MergePlan]:
    """``clean_label`` resolves the fragment label via the library's variant list. If the
    resulting key equals the stored key, nothing to merge (caller shouldn't have passed it
    in — kept defensive/consistent with the 012 clone's shape)."""
    plans = []
    for n in nodes:
...
def run(session: Session, *, apply: bool) -> MergeReport:
    ...
    plans = plan_merges(find_mismatched_nodes(session))
...
def check(session: Session) -> int:
    """Zero-write: report how many nodes still have a canonical-key mismatch (0 = done)."""
    remaining = find_mismatched_nodes(session)
    if remaining:
        logger.warning("%d technique_nodes with a canonical-key mismatch remain:", len(remaining))
        for n in remaining:
            logger.warning("  %s -> %r", n.node_key, n.label)
    else:
        logger.info("0 canonical-key mismatches remain — clean")
    return len(remaining)


def _print_report(report: MergeReport, *, apply: bool) -> None:
    verb = "MERGED" if apply else "WOULD MERGE"
    logger.info("%d technique_nodes with a canonical-key mismatch found", len(report.plans))
    for p in report.resolved:
        logger.info("  %s: %s -> %s", verb, p.attempt_key, p.canonical_key)
    if report.unresolved:
        logger.warning(
            "%d unresolved (clean_label left them unchanged — shouldn't happen by "
            "construction, investigate):", len(report.unresolved)
        )
...
def main() -> int:
    ...
    ap = argparse.ArgumentParser(description="Merge synonym-fragmented technique_nodes")
    mode = ap.add_mutually_exclusive_group()
    mode.add_argument("--apply", action="store_true", help="write (default: dry-run preview)")
    mode.add_argument(
        "--check", action="store_true", help="report remaining mismatched nodes, no writes"
    )
