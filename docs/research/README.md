...
| `next_moves_literature.md` | Literature pass for the next-move predictor (backlog `literature-pass-next-moves`, 2026-09-11): 17-source findings table, what PoC-E9 settled vs the pre-registered hypotheses H1-H5 (chronological split, log-loss thresholds), explicit non-goals. Carries the measured finding that the shipped 27.2% top-3 is 5.6 pp optimistic under a random-by-bout split. |
| `2026-09-14_e0_ksweep_whr.md` | PoC-E0 extension: the Elo K sweep past 80 (backlog `k-sweep-alem-80`) and Whole-History Rating (PoC-E3, backlog `poc-e3-whr`), both through the E0 harness on both corpora. Pre-registration, method, generated tables, verdicts. **K curve turns at K=100 on both corpora but the bottom is flat — no production K change; WHR rejected.** Runner `scripts/research/e0_ksweep_whr.py`; the block between the `<!-- E0KWHR:RESULTS -->` markers is generated, do not hand-edit. |
| `2026-09-18_entity_probe_prereg.md` | PoC-E17 pre-registration: evaluates the owner's entity-probe proposal (bbox→pair crop→DINO→light head) against what the repo actually has (`diag_seed_probe.py`, `crops.py`, Harmony v3 manifests). No box↔identity annotation exists yet — reduces the proposal to one decidable branch (pair-crop DINOv2 vs. frame-wide ConvNeXt vs. majority, match-grouped split), death-rule gated, not yet run. |
| `poc/` | Executed PoC results: `e0.md` (generated tables) + `e0_notes.md` (reading); `e8.md` (interaction graph) and `e4.md` (PtV γ/shaping calibration + Markov-order probe + the greedy-route naming audit) — criterion, tables, reading and verdict, all generated by their runner (`analysis/poc/e8_interaction_graph.py`, `analysis/poc/e4_ptv_eval.py`); do not hand-edit. |
