...
|---|---|---|---|
| 17 | Site generator emits percent/pp conflations, undefined metrics, n=1 headline rates, runtime-patched wrong prose, self-normalised radars, corpus-vs-career framing, raw-ELO leaks — catalogue in `GrapplingArc/docs/RESEARCH_NOTES.md`. | Analytics `export/site_data.py` → site | **PoC-S1** (route through `gated`/`est`, generated methodology page); fabricated fixture (`leandro-lo-vs-gordon-ryan-2025`) removed from `scripts/dumps/leandro_lo_events.json` 2026-09-11 — still lives in the `matches` row it was imported into and in the generated `site/` bundle until the row is deleted from prod and both athletes replayed (Decision 7, 2026-09-03) |
