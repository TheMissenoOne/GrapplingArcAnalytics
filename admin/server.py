...
def create_admin_app() -> FastAPI:
    ...
    def study_page(request: Request) -> Any:
        ...
        from admin.study import list_reports, load_knowledge

        return templates.TemplateResponse(
            request,
            "study.html",
            context={"knowledge": load_knowledge(), "reports": list_reports()},
        )
    ...
    @app.post("/admin/study/reports")
    async def study_save_report(request: Request) -> Any:
        from admin.study import REPORTS_DIR, save_report

        try:
            payload = await request.json()
            if not isinstance(payload, dict):
                raise ValueError("Report payload must be an object")
            report = save_report(payload, REPORTS_DIR)
        except (ValueError, TypeError) as exc:
            return JSONResponse({"error": {"message": str(exc)}}, status_code=400)
        except OSError as exc:
            return JSONResponse(
                {"error": {"message": f"Could not save report: {exc}"}}, status_code=500
            )
        return JSONResponse(report, status_code=201)

    @app.get("/admin/study/reports")
    def study_reports() -> Any:
        from admin.study import list_reports

        return JSONResponse({"reports": list_reports()})

    @app.get("/admin/study/reports/{report_id}.{extension}")
    def study_report_file(report_id: str, extension: str) -> Any:
        from admin.study import REPORTS_DIR

        if extension not in {"html", "json"} or not re.fullmatch(r"[A-Za-z0-9_-]+", report_id):
            raise HTTPException(status_code=404, detail="Report not found")
        path = REPORTS_DIR / f"{report_id}.{extension}"
        if not path.is_file():
            raise HTTPException(status_code=404, detail="Report not found")
        media_type = "text/html" if extension == "html" else "application/json"
        return FileResponse(
